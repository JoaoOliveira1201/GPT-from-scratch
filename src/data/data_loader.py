...
class DataLoader:
    ...
    def load_data(self, file_paths: List[str]):
        ...
        for path in file_paths:
            if os.path.isfile(path):
                ...
                mlflow_logger.warning(f"Path not found or invalid type: {path}")
        ...
        mlflow_logger.info(f"Encoding {len(combined_text)} characters of text...")
    ...
    def _read_file(self, file_path: str) -> str:
        ...
        if not file_path.endswith(allowed_exts):
            mlflow_logger.debug(f"Skipping unsupported file type: {file_path}")
            return ""
    ...
    def _read_directory(self, dir_path: str) -> str:
        dir_texts = []
        mlflow_logger.info(f"Scanning directory: {dir_path}")
    ...
    def get_batch(self, split):
        ...
        if len(data_tensor) <= model_config.block_size:
            raise ValueError(
                f"Dataset too small for block_size {model_config.block_size}"
            )
